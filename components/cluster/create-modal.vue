       ...
       <div class="fixed inset-0 transition-opacity bg-opacity-50 bg-H1" aria-hidden="true"></div>
        ...
        <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-6 sm:align-middle sm:max-w-xl sm:w-full">
          <form
            class="p-6"
          >
            <h3 v-if="!isEdit" class="mb-6 ml-4 text-xl font-semibold text-primary">New Cluster</h3>
            <h3 v-else class="mb-6 ml-4 text-xl font-semibold text-primary">Edit Cluster</h3>
...
<script>
  export default {
    ...
    props: ['isEdit', 'selectedCluster', 'centers'],
    methods: {
      close() {
      ...
      submitForm () {
        ...
        formdata.append('clusterId', this.centerId)

        if (this.isEdit) {
          this.$axios.$post('backend/cluster/update/' + this.id,
            formdata)
            .then(response => {
              if (response.message) {
          ...
          this.$axios.$post('backend/cluster/create',
            formdata)
            .then(response => {
              if (response.message) {
                this.$toast.error(response.message)
    ...
    created() {
      if (this.isEdit) {
        this.id = this.selectedCluster.id
        this.name = this.selectedCluster.name
        this.clue = this.selectedCluster.clue
        this.centerId = this.selectedCluster.center.id
      } else {
        this.id = ''
        this.name = ''
        this.clue = ''
