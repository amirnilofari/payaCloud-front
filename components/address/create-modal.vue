      ...
      <div class="flex items-end justify-center w-full min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-6 sm:align-middle sm:max-w-xl sm:w-full">
          <form
            class="p-6"
          >
            <h3 v-if="!isEdit" class="mb-6 ml-4 text-xl font-semibold text-primary">New IP</h3>
            <h3 v-else class="mb-6 ml-4 text-xl font-semibold text-primary">Edit IP</h3>
...
<script>
  export default {
    ...
    props: ['isEdit', 'selectedAddress', 'packages'],
    methods: {
      close() {
      ...
      submitForm () {
        ...
        formdata.append('netmask', this.netmask)
        formdata.append('packageId', this.packageId)

        if (this.isEdit) {
          this.$axios.$post('backend/address/update/' + this.id,
            formdata)
            .then(response => {
              if (response.message) {
          ...
          this.$axios.$post('backend/address/create',
            formdata)
            .then(response => {
              if (response.message) {
                this.$toast.error(response.message)
    ...
    created() {
      if (this.isEdit) {
        ...
        this.gateway = this.selectedAddress.gateway
        this.netmask = this.selectedAddress.netmask
        this.packageId = this.selectedAddress.package.id
      } else {
        this.id = ''
        this.address = ''
        this.gateway = ''
